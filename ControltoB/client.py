...
def SetSocket():
    ...
    host = host_en.get()
    if host is None:
        tkinter.messagebox.showinfo("提示", "IP错误！")
        return
    hs = host.split(":")
    if len(hs) != 2:
        tkinter.messagebox.showinfo("提示", "IP错误！")
...
def BindEvents(canvas):
    ...
    def EventDo(data):
        soc.sendall(data)
    # 鼠标左键按下起来，ascii码100为d；ascii码117为u
    def LeftDown(e):
        print(e.x, e.y)
...
host_lab = tkinter.Label(root, text="服务端IP:端口  ")
host_en = tkinter.Entry(root, show=None, font=('Arial', 14), textvariabl=val)
sca_lab = tkinter.Label(root, text="窗口大小:")
sca = tkinter.Scale(root, from_=10, to=100, orient=tkinter.HORIZONTAL, length=100,showvalue=100, resolution=0.1, tickinterval=50, command=SetScale)
